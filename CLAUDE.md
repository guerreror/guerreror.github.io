...
bash test/integration_bootstrap_compat.sh     # run ONE integration test (any of the five in test/)
...
- `unit-tests.yml` — style contract plus the remaining `test/integration_*.sh` scripts (`plugin_toggles`, `bootstrap_compat`, `upgrade_cli`, `css_minify`, `new_plugins`). `comments` and `distill` were removed along with the demo blog posts they depended on.
